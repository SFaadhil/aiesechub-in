...
| RnR 26.2 | RnR **hero** recoloured amber → hot rod red on black to match ZiddiMania: ground `#0a0000 → #2b0406 → #0a0000`, glows `rgba(204,0,0,.22)` / `rgba(228,0,0,.10)`, accent `#ff3d3d` (`HERO_RED`) with `#cc0000` (`HERO_RED_DEEP`) for fills and borders. Reds sampled from the logo artwork, which is pure red — `#cc0000` dominant, `#e40000` highlight. `#ff3d3d` chosen over `#cc0000` for text so 11px labels hold 5.3–5.9:1 on the near-black ground. Scoped to `HeroSection` only: `ACCENT` (`#f59e0b`) still drives the dashboard chips, the tier medal ramp keeps gold/silver/bronze, and `.rnr-pending-badge` stays amber |
